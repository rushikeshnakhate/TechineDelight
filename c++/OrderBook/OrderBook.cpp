...
OrderType getOrderType(const std::string &orderType) {
    try {
        ...
        } else if (orderType == OB::ICE) {
            return OrderType::ICE;
        } else {
...
struct Order {
    ...
    Order(OrderId orderId, const Quantity &quantity, const Price &price, const Side &side)
            : orderId(std::move(orderId)), quantity(quantity), price(price), leavesQuantity(quantity), side(side),
              displaySize(quantity), totalQuantity{quantity + displaySize} {}
...
class IcekOrderTypeProcessor : public OrderTypeProcessor {
public:
    bool match(const OrderHolder &pOrder, const OrderHolder &oppositeOrder) override {
        if ((pOrder->side == Side::BUY && pOrder->price >= oppositeOrder->price) ||
            (pOrder->side == Side::SELL && pOrder->price <= oppositeOrder->price)) {
            MatchingEngine::mayBeFullyTraded(pOrder, oppositeOrder);
            MatchingEngine::mayBePartiallyTraded(pOrder, oppositeOrder);
            Report::ReportTrading(oppositeOrder->quantity-oppositeOrder->leavesQuantity, oppositeOrder->price);
            return true;
        }
        return false;
    }

    static void setDisplaySize(const OrderHolder &pNewOrder, const OrderType &orderType, const Requests &requests);

    static void addIceOrderToBook(const OrderHolder &pNewOrder);
};

void IcekOrderTypeProcessor::setDisplaySize(const OrderHolder &pNewOrder,
                                            const OrderType &orderType, const Requests &requests) {
    const int MIN_FIELDS_IN_REQUEST = 7;
    if (orderType != OrderType::ICE && pNewOrder != nullptr && requests.size() < MIN_FIELDS_IN_REQUEST) return;
    pNewOrder->displaySize = stoi(requests[6]);
    pNewOrder->totalQuantity = pNewOrder->quantity + pNewOrder->displaySize;
}

void IcekOrderTypeProcessor::addIceOrderToBook(const OrderHolder &pNewOrder) {
    if (!pNewOrder->isOrderClosed()) {
        if (pNewOrder->leavesQuantity != pNewOrder->quantity) {
            //bad ideal to change original quantity so updated leavesQuantity instead
            pNewOrder->totalQuantity = pNewOrder->leavesQuantity;
            pNewOrder->leavesQuantity = pNewOrder->displaySize;
        }
        Database::addToBook(pNewOrder);
    } else {
        pNewOrder->leavesQuantity = pNewOrder->displaySize;
        OrderHolder copyOfOrder = pNewOrder;
        Database::removeFromBook(pNewOrder);
...
public:
    static unique_ptr<OrderTypeProcessor> get(const OrderType &orderType) {
        try {
            ...
            else if (orderType == OrderType::ICE)
                return make_unique<IcekOrderTypeProcessor>();
            else {
...
bool SubAction::process(const Requests &requests) {
    try {
        ...
        //Now it seems its wrong place to initiate the order
        // Each order should be initiated in its orderType class
        IcekOrderTypeProcessor::setDisplaySize(newOrder, orderType, requests);
        ...
        if (orderType == OrderType::ICE)
            IcekOrderTypeProcessor::addIceOrderToBook(newOrder);
        return true;
    } catch (std::exception &ex) {
        // std::cout << __func__ << ":" << ex.what() << std::endl;
    }
